...
def evaluate_models(folder_path, sequence_length=100, sample_rate=0.01, batch_size=64, epochs=1):
    ...
    for file_name in tqdm(os.listdir(folder_path), desc="Loading files"):
        if file_name.endswith('.wav'):
            ...
            # Sampling down to 0.1%
            sampled_indices = np.random.choice(len(data), size=int(len(data) * sample_rate), replace=False)
